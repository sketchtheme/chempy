...
def solveEquation(equation):
    ...
    N = np.array(x[0])  # Get the first nullspace vector

    # Calculate coefficients
    coefficients = N / np.min(np.abs(N))
    coefficients = coefficients.astype(int)

    # Apply coefficients to the terms
    balanced_lhs = [f"{coefficients[i]} {term}" for i, term in enumerate(lsplits)]
    balanced_rhs = [f"{coefficients[i]} {term}" for i, term in enumerate(rsplits)]

    # Combine terms
    balanced_equation = f"{join(balanced_lhs, ' + ')} -> {join(balanced_rhs, ' + ')}"
    
    return balanced_equation

balanced_equation = solveEquation("(CrO)4 + Pb5N2 -> Pb2(CrO)4 + N3")
